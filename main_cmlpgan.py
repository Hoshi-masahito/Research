...
Original file is located at
    https://colab.research.google.com/drive/1t2KEkpZ0s_mzV_14MpFkT4rd78hlLnep
"""

!ln -s "/content/drive/My Drive/特別研究/mnist_sampled" "./"
    ...
    for i, (imgs, labels) in enumerate(dataloader):
        ...
        if epoch % 20 == 0:
          for j in range(img_num):
            noise = torch.FloatTensor(np.random.normal(0, 1, (N_Class**2, args.latent_dim))).cuda()
            ...
            y_fixed = y_fixed.scatter_(1,y_.view(N_Class**2,1),1).cuda()
            for i in range(10):
              with torch.no_grad():
                gen_imgs = generator(noise, y_fixed).view(-1,C,H,W)
              save_gen_imgs = gen_imgs[i*10]
              save_image(save_gen_imgs, img_save_path + '/epochs:%d/%d/epoch:%d-%d_%d.png' % (epoch, i, epoch,i, j), normalize=True)

print('finish!')

!zip -r /content/images-cGAN-90.zip /content/images-cGAN-90/

from google.colab import files
files.download("/content/images-cGAN-90.zip")

'''import shutil

directory = './images-cGAN-600'
...
directory = './drive/My Drive/images-cGAN-600'
